    ...
    <div v-for="group in groupedTransactions" :key="group.date" class="date-group">
      <div class="date-header">
        ...
        <div class="date-summary">
          <span v-if="group.income > 0" class="text-blue">+{{ formatCurrency(group.income) }}원</span>
          <span v-if="group.expense > 0" class="text-red">-{{ formatCurrency(group.expense) }}원</span>
        </div>
      </div>
...
.col-detail { flex: 3; }
.col-category { flex: 1.5; text-align: center; }
...
.empty-msg i { font-size: 3rem; color: #ddd; }
